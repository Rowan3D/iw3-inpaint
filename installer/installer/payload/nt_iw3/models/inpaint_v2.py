...
def take_frames(t, keep):
    """The frames `keep` names (positions in the window), in order."""
    if isinstance(keep, slice):
        return t[keep]
    idx = keep if torch.is_tensor(keep) else torch.as_tensor(list(keep), dtype=torch.long)
...
class NTInpaintV2(I2IBaseModel):
    ...
    def infer(self, x, mask, closing=False, inner_dilation=0, outer_dilation=0, base_width=None,
        ...
        return self.forward(x, mask, skip_i2i_offset=True, keep=keep, ids=ids, cache=cache)

    def _forward(self, x, mask, keep=None, ids=None, cache=None):
        # No temporal block in the image model: a frame nobody keeps is never
        # looked at, so drop it at the door. `ids`/`cache` mean nothing here and
        # are accepted only so the two models take the same call.
        if keep is not None:
            x, mask = take_frames(x, keep), take_frames(mask, keep)
    ...
    def forward(self, x, mask, skip_i2i_offset=False, keep=None, ids=None, cache=None):
        """`keep` = the frame positions the caller wants OUT (None = all of them).
        The input stays the whole window -- the video model's temporal blocks mix
        every frame in it -- but everything that is a per-frame function of the
        window is run for those frames only, and so is the composite."""
        src = x
        x = (x - 0.5) / 0.5
        ...
        x = self._forward(x, mask, keep=keep, ids=ids, cache=cache)

        if pad1 or pad2:
            x = F.pad(x, (0, -pad1, 0, -pad2))
            mask = F.pad(mask, (0, -pad1, 0, -pad2))
        if keep is not None:
            src, mask = take_frames(src, keep), take_frames(mask, keep)
